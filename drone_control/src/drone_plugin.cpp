...
namespace gazebo
{
  ...
  GazeboRosDrone::GazeboRosDrone() {}

  GazeboRosDrone::~GazeboRosDrone() {}

  // Load the controller
  void GazeboRosDrone::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf)
  {
    ...
    {
      ROS_INFO_NAMED("drone", "DronePlugin missing <robotNamespace>, "
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <commandTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <odometryTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <odometryFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <robotBaseFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <odometryRate>, "
          "defaults to %f",
    ...
    {
      ROS_WARN_NAMED("drone", "DronePlugin (ns = %s) missing <cmdTimeout>, "
          "defaults to %f",
    ...
    {
      ROS_FATAL_STREAM_NAMED("drone", "DronePlugin (ns = " << robot_namespace_
    ...
    ROS_DEBUG_NAMED("drone", "OCPlugin (%s) has started",
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&GazeboRosDrone::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ =
      boost::thread(boost::bind(&GazeboRosDrone::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboRosDrone::UpdateChild, this));

  }

  // Update the controller
  void GazeboRosDrone::UpdateChild()
  {
#ifdef ENABLE_PROFILER
    IGN_PROFILE("GazeboRosDrone::UpdateChild");
  ...
  void GazeboRosDrone::cmdVelCallback(
  ...
  void GazeboRosDrone::QueueThread()
  {
  ...
  void GazeboRosDrone::publishOdometry(double step_time)
  {
    ...
    linear.Z() = (pose.Pos().Y() - last_odom_pose_.Pos().Z()) / step_time;
  ...
  GZ_REGISTER_MODEL_PLUGIN(GazeboRosDrone)
}